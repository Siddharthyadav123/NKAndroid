package com.netkoin.app.custom_views.parallex;
...
public class ParallaxListViewHelper implements OnScrollListener {
    ...
    private View toolbarBackgroudView;
            ...
            if (listView.getChildCount() > 0) {
               ...
               // System.out.println(">>first item y= " + parallaxedView.getView().getY());
                ...
                if (toolbarBackgroudView != null)
                    updateToolbarBgAlpha(parallaxedView.getView().getY());
    ...
    private void updateToolbarBgAlpha(float y) {
        if (y >= 0f) {
            toolbarBackgroudView.setAlpha(0f);
        } else {

            float alpha = Math.abs(y) / 100;
            toolbarBackgroudView.setAlpha(alpha);
        }
    ...
    public void setToolbarBackgroudView(View toolbarBackgroudView) {
        this.toolbarBackgroudView = toolbarBackgroudView;
    }
}
